...
import 'package:sinirbozucu/sqlite/veritabani_yardimcisi.dart';
...
class KisilerDaoRepository {
  ...
  Future<List<Kisiler>> kisileriYukle() async {
    var db = await VeritabaniYardimcisi.veritabaniErisim();
    List<Map<String, dynamic>> maps = await db.rawQuery(//sorgu liste getirecek
        "SELECT * FROM kisiler");
  ...
  Future<List<Kisiler>> ara(String aramaKelimesi) async {
    var db = await VeritabaniYardimcisi.veritabaniErisim();
    List<Map<String, dynamic>> maps = await db.rawQuery("SELECT * FROM kisiler WHERE kisi_ad like '%$aramaKelimesi%'");//sorgu oluşturulması

    return List.generate(maps.length, (i) {
      var satir = maps[i];//satir değerlerinin alınması
     //burası kaç kere çalışırsa o kadar kişio luşmuştur anlamına gelir.
